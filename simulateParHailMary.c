	...
	for (int i = 0; i < graph->numNodes; i++) {
		...
		for (int j = 0; j < node->numAgents; j++) {
			...
			if (node->agentsInNode[j]->disposition == true && node->agentsInNode[j]->infectedEncounters >= 10) {
				...
				//data racing
				//actually i only think I needed this when I had the inner loop parallel
				node->numInfected++; 
			}
...
void moveAgent(struct Agent* agents, int numAgents, struct Graph* graph, PendingMove* moves, int *directions){
...
void simulateDay(int days, struct Graph* graph,int numAgents, struct  Agent* agents, int* dailyInfectedCounts){
	...
	for(int i = 0; i<days; i++){
		...
		moveAgent(agents, numAgents, graph, moves, directions);
